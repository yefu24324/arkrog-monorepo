...
description: 为用户明确指定的 ArknightsGameData JSON 文件创建或更新 @arkrog/arknights-schema。尚未覆盖的数据表使用脚本进行一次性全量 Bootstrap；已有 Schema 的游戏更新必须依据 analyze-json 报告由大模型局部修改具体 TypeScript 类型、Zod Schema、注释、Markdown 文档与测试，禁止重新全量生成。用户要求“生成 Schema”“为 JSON 添加类型”“根据变化指南更新类型”时使用。目标 JSON 文件为必选输入，类型变化指南为可选输入。
---
...
- `types`、`schemas` 与 `docs/types` 按原 JSON 路径镜像组织。
...
- 把人工语义元数据写入生成工具或可持久化来源，避免重新生成后丢失。
- 每个 Type/Schema 逻辑模块提供一份同名中文 Markdown，模块内每个定义使用独立章节并保留各自人工说明区。
...
3. 使用脚本一次性生成完整类型、Schema、目录索引、类型一致性测试和文档。
...
3. 只打开报告涉及的类型、Schema、依赖类型、对应 Markdown 和测试。
4. 根据完整真实数据确认局部变化边界；不要因为单条错误扩大修改范围。
5. 直接编辑现有源码，保留所有人工注释，并同步更新 TypeScript 类型、Zod、`.describe()`、MD 和类型断言。
