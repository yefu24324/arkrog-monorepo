/**
 * docs 与 frontend 共用的藏品包装数据契约。
 * `relic` 与 `charBuffs` 始终保留 GameData 原始对象，用户态字段只放在外层。
 */

import TinyPinyin from "tiny-pinyin";

import type { BlackboardValue, RelicItem } from "../classify/types.js";
...
export type ExportedRelicItemData = Omit<RelicItem, "usage"> & { usage: string };
...
export function buildWrappedRelicTopicArtifact(input: {
  ...
  const characterBuffs = Object.values(input.detail.charBuffData);
