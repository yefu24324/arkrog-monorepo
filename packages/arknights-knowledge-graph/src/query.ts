...
import { FIELD_MAPPINGS } from "./domain/damage-zones.js";
...
export async function traceItemWithConnection(
  ...
  const rows = await parameterizedRows(
    ...
    `MATCH (i:Item)-[:ITEM_HAS_EFFECT]->(e:Effect)
     ...
     RETURN i.rawId AS itemId, i.name AS itemName, e.key AS effectKey,
            e.parameters AS parameters, e.jsonPath AS jsonPath,
            z.id AS zoneId, z.name AS zoneName, mapping.status AS status,
            mapping.reason AS reason, mechanic.name AS mechanicName,
            mechanic.componentTypes AS componentTypes
  ...
  return rows.map((row) => ({
    ...
    reason: stringValue(
      row.reason,
      "当前规则和证据不足；黑板已保留，需补充战斗组件或人工文档映射。",
    ),
...
function formatTrace(rows: TraceRow[]): string {
  ...
  const details = rows.map((row, index) => {
    const mechanic = row.mechanicName
      ...
      : "";
    return `${index + 1}. ${row.effectKey} [${row.parameters || "无参数"}] → ${row.zoneName}（${row.zoneId}，${row.status}）${mechanic}\n   ${row.reason}\n   ${row.jsonPath}`;
  });
...
export async function answerQuestion(
  ...
  const mentionedField = [...FIELD_MAPPINGS.keys()]
